...
// complete
struct JumpB : Inst {  //用于生成有条件跳转指令的汇编代码。
  LoongArch::Block *target;
  Reg lhs, rhs;
  JumpB(Block *_target, Reg _lhs, Reg _rhs) : target(_target), lhs(_lhs), rhs(_rhs) { target->label_used = true; }

  virtual bool side_effect() override { return true; }
  virtual void gen_asm(std::ostream &out) override {
    out << "beq " << lhs << ", " << rhs << ", " << target->name << '\n';
  }
};
